...
// créer un detailcommande
// quantité
// attacher le produit
// attacher la commande
// persist
$detailCommande = new cava\DetailCommande();
$detailCommande->setQuantite(5);
...
// ajouter ce detailCommande à la commande via ArrayCollection
$commande->ajouterDetailCommande($detailCommande);

// calculer le montant HTVA de la commande
$tabDetail = $commande->getDetailCommandes()->toArray();
$montant = 0;
for ( $i = 0 ; $i < count($tabDetail); $i++ ){
$montant += $tabDetail[$i]->getQuantite() *  
$tabDetail[$i]->getProduit()->getPrixHTVA() *
(1 + $tabDetail[$i]->getProduit()->getTauxTVA());
}
echo $montant;
